...
public class CollectionService {
    ...
    public CollectionResponse createCollection(UUID userId, CreateCollectionRequest request) {
        ...
        collection.setVisibility(request.getVisibility() != null ? request.getVisibility() : VisibilityType.PUBLIC);
    ...
    @Transactional
    public CollectionResponse addGame(UUID collectionId, UUID userId, AddGameToCollectionRequest request) {
        Collection collection = collectionRepository.findById(collectionId)
                .orElseThrow(() -> new ResourceNotFoundException("Collection not found"));
        requireOwner(collection, userId);
    ...
    public List<CollectionSummaryResponse> getUserCollections(String username, UUID requestingUserId) {
        userRepository.findByUsername(username)
                .orElseThrow(() -> new ResourceNotFoundException("User not found"));

        List<VisibilityType> visibilities = resolveVisibilities(username, requestingUserId);
        List<Collection> collections = collectionRepository.findByOwnerUsernameAndVisibilityIn(username, visibilities);
        return buildSummaries(collections);
    }
        ...
        return userRepository.findByUsername(ownerUsername)
                .map(owner -> {
                    if (owner.getId().equals(requestingUserId)) {
                        return List.of(VisibilityType.PUBLIC, VisibilityType.FRIENDS, VisibilityType.PRIVATE);
                    }
    ...
    private void checkReadAccess(Collection collection, UUID requestingUserId) {
        ...
        if (v == VisibilityType.FRIENDS && friendshipRepository.existsByUserIdAndFriendId(requestingUserId, ownerId)) return;
    ...
    private List<CollectionSummaryResponse> buildSummaries(List<Collection> collections) {
        return collections.stream()
                .map(c -> {
                    List<CollectionItem> items = collectionItemRepository.findByCollectionIdWithGame(c.getId());
                    List<String> previewCoverIds = items.stream()
