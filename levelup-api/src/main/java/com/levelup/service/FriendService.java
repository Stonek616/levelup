...
public class FriendService {
  ...
  public FriendRequestActionResponse sendFriendRequest(
      UUID requesterId, SendFriendRequestRequest request) {
  ...
  @Transactional
  public FriendRequestActionResponse respondToRequest(
      UUID requestId, UUID receiverId, RespondToFriendRequestRequest request) {
  ...
  public Page<FriendshipResponse> getFriends(UUID userId, Pageable pageable) {
    return friendshipRepository.findByUserId(userId, pageable).map(FriendshipResponse::from);
  }
  ...
  public void removeFriend(UUID userId, UUID friendUserId) {
    Friendship aToB =
        friendshipRepository
            .findByUserIdAndFriendId(userId, friendUserId)
            .orElseThrow(() -> new ResourceNotFoundException("Friendship not found"));
    friendshipRepository.delete(aToB);

    friendshipRepository
        .findByUserIdAndFriendId(friendUserId, userId)
        .ifPresent(friendshipRepository::delete);

    friendRequestRepository
        .findByRequesterIdAndReceiverId(userId, friendUserId)
  ...
  public String getRequestStatus(UUID currentUserId, UUID targetUserId) {
    return friendRequestRepository
        ...
        .or(
            () ->
                friendRequestRepository.findByRequesterIdAndReceiverId(targetUserId, currentUserId))
