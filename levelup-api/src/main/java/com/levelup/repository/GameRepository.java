...
public interface GameRepository extends JpaRepository<Game, UUID> {
  ...
  @Query(
      "SELECT g FROM Game g LEFT JOIN FETCH g.genres LEFT JOIN FETCH g.themes WHERE g.slug = :slug")
  ...
  Page<UUID> findThemeBasedRecommendations(
      @Param("userId") UUID userId, @Param("platformName") String platformName, Pageable pageable);
  ...
  @Query(
      "SELECT genre.name FROM Game g JOIN g.genres genre GROUP BY genre.id, genre.name HAVING COUNT(g.id) >= :minCount ORDER BY genre.name")
