...
public class ReviewService {
  ...
  public Page<ReviewResponse> getReviewsForGame(
      UUID gameId, UUID currentUserId, Pageable pageable) {
  ...
  public ReviewResponse updateReview(UUID reviewId, UUID userId, UpdateReviewRequest request) {
    ...
    if (request.getRating() != null) {
      libraryEntryRepository
          .findByUserIdAndGameId(userId, review.getGame().getId())
          .ifPresent(
              entry -> {
  ...
  public Page<ReviewResponse> getReviewsByUser(
      String username, UUID currentUserId, Pageable pageable) {
    User user =
        userRepository
    ...
    if (!isOwn) {
      VisibilityType vis = user.getReviewsVisibility();
