...
export class LibraryService {
  ...
  getLibrary(filters?: {
    ...
    if (filters?.ownership != null)
      params = params.set('ownership', filters.ownership);
    ...
    return this.http.get<PagedResponse<LibraryEntry>>(
  ...
  addToLibrary(request: CreateLibraryEntryRequest): Observable<LibraryEntry> {
    return this.http.post<LibraryEntry>(
      `${environment.apiUrl}/library`,
      request,
    );
  }
  ...
  updateEntry(
    ...
    return this.http.patch<LibraryEntry>(
      `${environment.apiUrl}/library/${entryId}`,
      request,
    );
  }
  ...
  getEntryForGame(gameId: string): Observable<LibraryEntry> {
    return this.http.get<LibraryEntry>(
      `${environment.apiUrl}/library/game/${gameId}`,
    );
  }
  ...
  getUserLibrary(
    ...
    if (filters?.size != null) params = params.set('size', filters.size);
    return this.http.get<PagedResponse<LibraryEntry>>(
      `${environment.apiUrl}/users/${username}/library`,
      { params },
    );
  }
}
