...
export class SettingsService {
  ...
  updateAccount(
    ...
    return this.http
      .patch<UserProfile>(`${environment.apiUrl}/settings/account`, request)
      .pipe(
        tap((updated) => {
  ...
  getGameProfiles(): Observable<GameProfile[]> {
    return this.http.get<GameProfile[]>(
      `${environment.apiUrl}/users/me/game-profiles`,
    );
  }
  ...
  removeGameProfile(profileId: string): Observable<void> {
    return this.http.delete<void>(
      `${environment.apiUrl}/users/me/game-profiles/${profileId}`,
    );
  }
}
