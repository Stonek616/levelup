 ...
 *
 * Fields like genres/themes/keywords stay as raw ID arrays — we have those
 * names in our own taxonomy tables already, no point pulling them again.
 *
 * Fields like platforms/game_modes/etc. are expanded so we get names directly,
 * and we store those denormalized as TEXT[] on the games row.
 *
 * @JsonIgnoreProperties(ignoreUnknown = true) means IGDB can return extra
 * fields we don't care about without breaking deserialization.
 */
...
public record IgdbGameDto(
    ...
    /**
     * involved_companies has a join structure — it tells you whether a company
     * was the developer, publisher, both, or neither. We only care about developers
     * for now (per the design doc — publishers don't drive recommendations).
     */
    @JsonIgnoreProperties(ignoreUnknown = true)
